...
function GameLayer:init(heroCount)
    ...
    cache:addSpriteFrames("num.plist")

    local num = cc.Sprite:createWithSpriteFrameName("num_00.png")
    self:addChild(num)
    num:setPosition(295, visibleSize.height - num:getContentSize().height / 2 - 80)

    local animFrames = {}
    for i = 0, 3 do 
        local frame = cache:getSpriteFrame(string.format("num_%02d.png", i) )
        animFrames[i] = frame
    end

    local animation = cc.Animation:createWithSpriteFrames(animFrames, 0.5)
    local animate = cc.Animate:create(animation);
    num:runAction(cc.RepeatForever:create(animate))
